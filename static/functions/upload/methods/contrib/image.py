...
image_fields = ['type', 'file_size', 'width', 'height']
...
def get_image_size(input, size):
    """
    Return (width, height) for a given img file content - no external
    dependencies except the os and struct builtin modules
    """
    img = get_image_metadata(input, size)
    return (img.width, img.height)


def get_image_metadata(input, size):
    height = -1
    width = -1
    data = input.read(26)
    ...
    return Image(type=imgtype, file_size=size, width=width, height=height)
